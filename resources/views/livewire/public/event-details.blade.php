        ...
        <div class="lg:grid lg:grid-cols-3 lg:gap-12">
            ...
            <div class="lg:col-span-2">
                ...
                <nav class="mb-6 flex items-center gap-2 text-sm text-gray-400">
                    <a href="{{ route('events.index') }}" class="hover:text-gray-600">Events</a>
                    @if($event->category)
                        <span>/</span>
                        <a href="{{ route('events.index', ['category' => $event->category->slug]) }}"
                           class="hover:text-gray-600">{{ $event->category->name }}</a>
                    @endif
                    <span>/</span>
                ...
                {{-- Title --}}
                <h1 class="text-3xl font-extrabold text-gray-900 sm:text-4xl">{{ $event->title }}</h1>

                @if($event->excerpt)
                <p class="mt-3 text-lg text-gray-600">{{ $event->excerpt }}</p>
                @endif
                ...
                <div class="mt-8 space-y-4">
                    <div class="flex items-start gap-3 text-sm text-gray-700">
                        <svg class="mt-0.5 h-5 w-5 shrink-0 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                        </svg>
                        <div>
                            <p class="font-medium">{{ $event->start_at->format('l, d F Y') }}</p>
                    ...
                    <div class="flex items-center gap-3 text-sm text-gray-700">
                        <svg class="h-5 w-5 shrink-0 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="flex items-start gap-3 text-sm text-gray-700">
                        ...
                        <div>
                            ...
                            @endif
                            @if($event->city)
                            <p class="text-gray-500">{{ $event->city }}</p>
                            @endif
                        </div>
                    </div>
                    @endif
                ...
                {{-- Organizer --}}
                @if($event->organizer)
                <div class="mt-10 rounded-xl border border-gray-100 bg-gray-50 p-5 flex items-center gap-4">
                    @if($event->organizer->logo_url)
                        <img src="{{ $event->organizer->logo_url }}" alt="{{ $event->organizer->display_name }}"
                             class="h-12 w-12 rounded-full object-cover border border-gray-200">
                    @else
                        <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-indigo-600 font-bold text-lg">
